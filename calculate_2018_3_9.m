...
% xlim([0,1000*T])
% title('背景测量数据时域波形')
% figure(2); plot(t*1000,X8,'k');
% xlabel('Time(ms)'); ylabel('Corona Current(mA)');
% % xlim([0,1000*T])
% axis([0,1000*T,-20,20])
% title('电晕电流测量数据时域波形')
...
%% 绘图
% figure(3)
% plot(f(1:1300),(abs_Y8(1:1300))); grid on;
% xlabel('频率(Hz)'); ylabel('幅值|Y(f)|');
% title('加压信号的单边幅值频谱')
% xlim([10,1300]);
...
% abs_Y8(1)=1e-6; 
% abs_Y8 = 10*log10(abs_Y8.^2/1e-12);
clear Y8; clear ave; 
clear abs_Y8;
...
% figure(3); plot(t*1000,denoise_result,'k'); 
% xlabel('Time(ms)'); ylabel('Corona Current(mA)');
% % xlim([0,1000*T])
% axis([0,1000*T,-20,20])
...
%对数能量谱
Y_new = fft(denoise_result,Nfft);
Y_new(1) = 0;    % 去除直流分量
abs_Y_new=2*abs(Y_new(nfft))/N;            %1024*1024个点

figure
size_font = 15;
h = gca; % 获取当前绘图坐标的指针
set(h,'FontSize',size_font);
plot(f(1:641)/1000,abs_Y_new(1:641),'k-')
% plot(f(1:641)/1000,abs_Y8(1:641),'b-')
xlabel('频率(kHz)','FontName','宋体','FontSize',16);
ylabel('离散频谱幅值(dB))','FontName','宋体','FontSize',16);
title('1000 kV离散频谱')


abs_Y_new(1)=1e-6; 
abs_Y_new = 10*log10(abs_Y_new.^2/1e-12);
...
f_f=f_f';
for ii=1:length(f_f)
 ave_power(ii)=mean(power(round((f_f(ii)-d_Hz/2)*1000*1000/Fs*Nfft):round((f_f(ii)+d_Hz/2)*1000*1000/Fs*Nfft)));    %用的是对数幅值分贝值
end
%计算分贝值
% ave_power(1)=1;  
plot_Pxx=10*log10(ave_power);  %取对数值   对能量求对数是干什么？
...
% plot(f(1:641)/1000,abs_Y8(1:641),'b-')
xlabel('频率(kHz)','FontName','宋体','FontSize',16);
ylabel('对数功率谱(dB))','FontName','宋体','FontSize',16);
title('700 kV对数功率谱')


figure
plot(f_f,plot_Pxx,'o-k','Markersize',1.5)
title('功率谱')
xlim([0.15,4])
xlabel('频率(MHz)'); ylabel('电晕电流对数能量谱(dB)');
